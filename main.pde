...
void draw() {
  ...
  if(step2)     
    {
    fill(yellow);
    if(live) {M.computeO(); roomba = M.Y(2);} // **02 implement it in Mesh
    if(showEdges) 
      {
      fill(yellow); 
  ...
  if(step4)
    {
    for(int i=0; i<5; i++) M.smoothenInterior(); // **04 implement it in Mesh
    M.writeVerticesTo(R);  
    }
 ...
 // **05 implement corner operators in Mesh
  ...
  int line=0;
  scribeHeader(" Project 3 for Rossignac's 2018 Graphics Course CS3451 / CS6491 by Elizabeth Elsa Perakis ",line++);
  scribeHeader(P.count()+" vertices, "+M.nt+" triangles, "+M.countBorders()+" border edges, "+"Length of path automated "+distance1+" steps "+"Length of path manual "+distance2,line++);
  if(live) scribeHeader("LIVE",line++);
  if (step8 && distance1>distance2) scribeHeader("Winner is the manual roomba!",line++);
  if (step8 && distance1<distance2) scribeHeader("Winner is the automated roomba!",line++);
